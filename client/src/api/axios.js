...
const api = process.env.REACT_APP_DB_URL || 'localhost:8080'
...
const fetchRequests = {
  ...
  getQuestions: async () => {
    const questions = []
    const questionsData = await axios.get('http://' + api + '/questions')
  ...
  getUserData: (userEmail) => {
    return axios.get('http://' + api + '/user', {
      params: {
        userEmail: userEmail
      }
    })
  ...
  updateUserScore: (userId, score) => {
    return axios.patch('http://' + api + '/user', {
      params: {
        userId: userId,
        score: score
      }
  ...
  getGroupData: (groupId) => {
    return axios.get('http://' + api + '/group', {
      params: {
        groupId: groupId
      }
    })
  ...
  joinGroup: async (userId, groupName) => {
    const groupId = await axios.patch('http://' + api + '/group', {
      params: {
        action: 'join',
        userId: userId,
        groupName: groupName
